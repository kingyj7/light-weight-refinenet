...
def get_arguments():
    ...
    parser.add_argument("--val-list", type=str, nargs='+', default=INFER_LIST,
...
def create_loaders(
    ...
    composed_trn = transforms.Compose([ResizeShorterScale(shorter_side, low_scale, high_scale),
                                    ...
                                    ToTensor()])
    composed_val = transforms.Compose([ResizeShorterScale(shorter_side, low_scale, high_scale),
                                    Normalise(*normalise_params),
                                    ToTensor()])
...
#           
    ...
    #miou = np.mean(ious)
    hiou = ious[1]
    logger.info(' Val epoch: {}\t Mean IoU: {:.3f}\t avg_Recall-hand : {:.3f}\t resize_scale: {:.1f}'.format(
                                epoch, hiou,recall_hand,args.low_scale[0]))
